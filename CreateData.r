...
create_ydata_file<-function(InstallNum=1){
    ...
    print(paste("create_ydata_file",InstallNum))
...
create_xdata_file<-function(InstallNum=1){
    # Create a file with a bunch of X data.  Based on get_data_asof.
    # This will be combined with Y data file.
    # create strings of field names to be extracted from various files. Then load the files. Then create new fields
    print(paste("create_xdata_file",InstallNum))
    if (InstallNum>length(sipbInstallDates)) return(NA)
    strDate<-sipbInstallDates[InstallNum]
